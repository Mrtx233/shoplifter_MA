...
base_url = 'http://mz.hg.gov.cn/'
url_name = '黄冈市民政局'
notification_path = '基层动态'
base_path = f'E:\WorkingWord\马缕_公文爬取(7.8-7.12)\民政局\湖北省\\{url_name}'
excel_file_name = f'{url_name}.xlsx'

page_list_xpath = '//*[@id="mainCon"]/div/div[2]/div[2]/div[3]/ul/li/a/@href'
tit_xpath = '//*[@id="wenzhang"]/h1/text()'
menu_xpath = '//div[@class="ls-crumbs"]//a//text()'
fb_time_xpath = '//*[@id="wenzhang"]/div[1]/div[1]/span[1]/text()'
ly_name_xpath = '//*[@id="wenzhang"]/div[1]/div[1]/span[3]/text()'
...
for i in range(101, 151):
    if i == 1:
        url = 'http://mz.hg.gov.cn/zwdt/jcdt/index.html'
    else:
        url = f'http://mz.hg.gov.cn/content/column/6792595?pageIndex={i}'
    ...
    for page in page_list:
        try:
            ...
            with open(file_path, 'a+', encoding='utf-8') as file:
                file.write(tit + '\n')
                for p_text in page_tree.xpath('//div[@id="wenzhang"]/div[2]/p'):
                    if p_text.xpath('.//text()') != '':
                        content = ''.join(p_text.xpath('.//text()'))
                        file.write(content + '\n')
                for p_text in page_tree.xpath('//div[@id="wenzhang"]/div[2]/content/p'):
                    if p_text.xpath('.//text()') != '':
                        content = ''.join(p_text.xpath('.//text()'))
                        file.write(content + '\n')

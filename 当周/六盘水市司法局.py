...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\六盘水市司法局'
# 通知公告路径
notification_path = '时政要闻'
# Excel文件名
excel_file_name = '六盘水市司法局.xlsx'
...
for i in range(1, 11):  # 循环生成分页URL
    url = f'https://sfj.gzlps.gov.cn/sfdt/szyw/index_{i}.html'  # 构建URL
    ...
    try:
        ...
        for page in page_list:  # 遍历每个页面链接
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('/html/body/div[11]/div[2]/div/div[1]/h1/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="current_wz"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('/html/body/div[11]/div[2]/div/div[1]/div/p/text()[2]')
                # 提取来源
                ly_name = page_tree.xpath('/html/body/div[11]/div[2]/div/div[1]/div/text()[2]')

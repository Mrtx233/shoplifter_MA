...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\六盘水市生态环境局'
# 通知公告路径
notification_path = '通知公告'
# Excel文件名
excel_file_name = '六盘水市生态环境局.xlsx'
...
for i in range(1, 11):  # 循环生成分页URL
    url = f'https://sthj.gzlps.gov.cn/gzdt_43233/tzgg_43236/index_{i}.html'  # 构建URL
    ...
    try:
        ...
        page_list = tree.xpath('//div[@class="main_rt_list"]/ul/li/a/@href')  # 提取每个页面的链接

        for page in page_list:  # 遍历每个页面链接
            time.sleep(random.uniform(1, 1.5))  # 随机等待1到1.5秒，避免被封IP
            page_url = page  #
            # page_url = 'https://www.xizang.gov.cn/xwzx_406/bmkx' + re.sub(r'\./', '/', page)
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('//*[@id="c"]/div[1]/div[1]/h2/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="dqwz_con"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('//*[@id="c"]/div[1]/div[2]/text()[2]')
                # 提取来源
                ly_name = page_tree.xpath('//*[@id="c"]/div[1]/div[2]/text()[3]')
                ...
                # 查找目标div
                target_div = soup.find('font', id='Zoom')

...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\新疆维吾尔自治区文化和旅游厅'
# 通知公告路径
notification_path = '文旅厅要闻'
# Excel文件名
excel_file_name = '新疆维吾尔自治区文化和旅游厅.xlsx'
...
for i in range(2, 11):  # 循环生成分页URL
    url = f'https://wlt.xinjiang.gov.cn/wlt/wltyw/list_{i}.shtml'  # 构建URL
    ...
    try:
        ...
        page_list = tree.xpath('//div[@class="lr_xxgk_con"]/ul/li/a/@href')  # 提取每个页面的链接
        ...
        for page in page_list:  # 遍历每个页面链接
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('/html/body/div[1]/div[3]/div[2]/div[1]/h1/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="local"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('/html/body/div[1]/div[3]/div[2]/div[2]/text()')
                # 提取来源
                ly_name = page_tree.xpath('/html/body/div[1]/div[3]/div[2]/div[2]/text()')
                ...
                # 查找目标div
                target_div = soup.find('div', id='vsb_content_501')

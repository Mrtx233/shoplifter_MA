...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\毕节市司法局'
# 通知公告路径
notification_path = '部门动态'
# Excel文件名
excel_file_name = '毕节市司法局.xlsx'
...
for i in range(71, 81):  # 循环生成分页URL
    url = f'https://www.bijie.gov.cn/bm/bjssfj/dt/bmdt/index_{i}.html'  # 构建URL
    ...
    try:
        ...
        for page in page_list:  # 遍历每个页面链接
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('/html/body/div[9]/div[2]/div[2]/div[1]/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="Address Box"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('/html/body/div[9]/div[2]/div[2]/div[2]/div[1]/p[1]/span[2]/text()')
                # 提取来源
                ly_name = page_tree.xpath('/html/body/div[9]/div[2]/div[2]/div[2]/div[1]/p[1]/span[1]/text()')

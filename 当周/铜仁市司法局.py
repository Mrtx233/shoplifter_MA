...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\铜仁市司法局'
# 通知公告路径
notification_path = '通知公告'
# Excel文件名
excel_file_name = '铜仁市司法局.xlsx'
# 合并基础路径和通知公告路径
...
for i in range(1, 10):  # 循环遍历第11到20页
    url = f'https://sfj.trs.gov.cn/tzgg/index_{i}.html'  # 构建URL
    ...
    page_list = tree.xpath('//div[@class="NewsList"]/ul/li/a/@href')  # 提取每个页面的链接
    ...
    for page in page_list:  # 循环遍历每个页面链接
        try:
            ...
            tit = page_tree.xpath('//*[@id="c"]/div[1]/h1/text()[1]')[0]  # 提取标题
            tit = re.sub(r'[\r\n\s\t]*', '', tit)  # 清理标题中的空白字符
            menu = '>'.join(page_tree.xpath('//div[@class="current_wz"]//a//text()'))  # 提取目录
            fb_time = page_tree.xpath('//*[@id="c"]/div[1]/div/span[1]/text()')  # 提取发布时间
            ly_name = page_tree.xpath('//*[@id="c"]/div[1]/div/span[2]/text()')  # 提取来源

            file_path = f'{folder_path}\\{tit}.docx'  # 构建文件路径
            file = open(file_path, 'a+', encoding='utf-8')  # 以追加模式打开文件
            file.write(tit + '\n')  # 写入标题

            content_p = page_tree.xpath('//font[@id="Zoom"]/div/p')  # 提取内容段落
            if len(content_p) > 0:  # 如果内容段落存在
                for p_text in content_p:  # 循环遍历每个段落
                    content = ''.join(p_text.xpath('.//text()'))  # 提取段落中的文本
                    file.write(content + '\n')  # 写入文件

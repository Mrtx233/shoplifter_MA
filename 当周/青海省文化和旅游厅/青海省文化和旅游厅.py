...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\青海省文化和旅游厅'
# 通知公告路径
notification_path = '地方风采'
# Excel文件名
excel_file_name = '青海省文化和旅游厅.xlsx'
...
for i in range(2, 11):  # 循环生成分页URL
    url = f'https://whlyt.qinghai.gov.cn/dffc/page-{i}.html'  # 构建URL
    ...
    try:
        ...
        page_list = tree.xpath('//div[@class="app-article-list"]/a/@href')  # 提取每个页面的链接

        for page in page_list:  # 遍历每个页面链接
            time.sleep(random.uniform(1, 1.5))  # 随机等待1到1.5秒，避免被封IP
            page_url = page  #
            # page_url = 'https://wlt.xinjiang.gov.cn' + re.sub(r'\./', '/', page)
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('//*[@id="app"]/div[3]/div/div/div[2]/div[1]/div[1]/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="app-breadcrumb"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('//*[@id="app"]/div[3]/div/div/div[2]/div[1]/div[2]/span[1]/text()')
                # 提取来源
                ly_name = page_tree.xpath('//*[@id="app"]/div[3]/div/div/div[2]/div[1]/div[2]/span[3]/text()')
                ...
                # 查找目标div
                target_div = soup.find('div', class_='app-content-body')

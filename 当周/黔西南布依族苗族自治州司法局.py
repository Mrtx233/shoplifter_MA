...
# 基础路径
base_path = r'E:\WorkingWord\马缕_公文爬取(7.3-7.5)\黔西南布依族苗族自治州司法局'
# 通知公告路径
notification_path = '通知公告'
# Excel文件名
excel_file_name = '黔西南布依族苗族自治州司法局.xlsx'
...
for i in range(1, 9):  # 循环生成分页URL
    url = f'https://sfj.qiannan.gov.cn/xwzx/tzgg/index_{i}.html'  # 构建URL
    ...
    try:
        ...
        page_list = tree.xpath('//ul[@class="text-list"]/li/a/@href')  # 提取每个页面的链接
        ...
        for page in page_list:  # 遍历每个页面链接
            ...
            try:
                ...
                # 提取文件名，去除多余的空格和换行
                file_name = page_tree.xpath('//*[@id="c"]/div[1]/text()')[0]
                file_name = re.sub(r'[\r\n\s\t]*', '', file_name)
                # 提取目录
                menu = '>'.join(page_tree.xpath('//div[@class="bt-position"]//a//text()'))
                # 提取发布时间
                fb_time = page_tree.xpath('//*[@id="c"]/div[3]/span[1]/text()')
                # 提取来源
                ly_name = page_tree.xpath('//*[@id="c"]/div[3]/span[2]/text()')
